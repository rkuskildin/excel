...
**GigaChat (Sber)** — особый случай (свой OAuth, не OpenAI-совместим): задаётся не через
`API_KEY`, а через `GIGACHAT_CREDENTIALS` (Authorization key из developers.sber.ru),
плюс `GIGACHAT_MODEL` (`GigaChat-2-Max`) и `GIGACHAT_SCOPE` (`GIGACHAT_API_PERS`). Если
`GIGACHAT_CREDENTIALS` задан — он становится основным провайдером (через нативный
`langchain-gigachat`), а OpenAI-провайдеры из `MODEL_FALLBACKS` идут как резерв.
